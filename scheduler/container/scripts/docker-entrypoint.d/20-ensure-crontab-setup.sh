...
if [ $? -ne 0 ]; then
    ...
    echo "#*0 /1 * * * /usr/bin/quackjob --job=\"calendar-events.json\" >> /quackview/logs/cron-run.log 2>&1" | crontab -u quackjob -
    echo "#0 4 /7 * * /usr/bin/quackjob --job=\"dad-jokes.json\" >> /quackview/logs/cron-run.log 2>&1" | crontab -u quackjob -
